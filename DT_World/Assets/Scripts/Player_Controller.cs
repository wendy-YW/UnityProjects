...
//     [SerializeField] WheelCollider rearRightWheelCollider;
    ...
    Color color_inactive = new Color(1,1,1,0);
    Color color_active = new Color(1,1,1,0.7f);
    ...
    private void OnTriggerEnter(Collider other){
        Debug.Log($"Player car collided with {other.gameObject.name}");
...
void UpdateStatus(int prev_index = -1){
    ...
    if(prev_index==-1){
        ...
        Renderer wp_renderer = this.wps[this.next_wp_index].GetComponent<Renderer>();
        wp_renderer.material.SetColor = this.color_active;
    }else{
        Debug.Log($"Player car passed waypoint {prev_index} and is now on waypoint {this.next_wp_index}");
         Renderer wp_renderer = this.wps[this.next_wp_index].GetComponent<Renderer>();
        wp_renderer.material.SetColor = this.color_inactive;
    }
}
